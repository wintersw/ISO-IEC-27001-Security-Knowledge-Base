...
description: Starter page for A.8.21 Security of network services; expand using the Annex A control page template.
category: Annex A
...
This control protects communication and technical trust boundaries. Implementation should be based on data flows, service criticality, identity, configuration standards, monitoring, and controlled exceptions.

For A.8.21, begin by identifying the specific risk, legal requirement, contractual commitment, or operational need that makes the control necessary. The control owner should then define what is in scope, which roles perform the activity, which systems or data sources are authoritative, how exceptions are handled, and what evidence proves that the control operated.
...
A critical database is reachable from broad internal networks. The team maps required flows, introduces segmentation, restricts administration paths, logs denied traffic, and verifies application functionality.
