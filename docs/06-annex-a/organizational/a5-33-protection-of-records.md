...
description: Practical implementation, evidence, and audit guidance for A.5.33.
category: Annex A
...
Records need protection against loss, unauthorized access, alteration, and premature destruction for as long as business or legal needs require. Controls should cover creation, indexing, storage, retrieval, retention, and disposal.
...
- **ISO requirement:** Annex A controls are not automatically mandatory. Determine necessary controls through risk treatment, record applicability in the Statement of Applicability, and implement A.5.33 when selected or otherwise committed to.
- **Implementation guidance:** Define **protection of records** through its owner, scope, trigger, workflow, exceptions, and evidence.
...
For A.5.33, begin by identifying the specific risk, legal requirement, contractual commitment, or operational need that makes the control necessary. The control owner should then define what is in scope, which roles perform the activity, which systems or data sources are authoritative, how exceptions are handled, and what evidence proves that the control operated.

## Practical example

Audit evidence is stored in a restricted repository with immutable version history, an approved retention period, named ownership, recoverable backups, and a disposal hold when litigation is anticipated.
