...
description: Practical implementation, evidence, and audit guidance for A.6.1.
category: Annex A
...
Screening checks whether a candidate's identity, qualifications, experience, and suitability are consistent with the trust a role requires. Checks must be lawful, proportionate, consistent, and protected as sensitive personal information.
...
- **ISO requirement:** Annex A controls are not automatically mandatory. Determine necessary controls through risk treatment, record applicability in the Statement of Applicability, and implement A.6.1 when selected or otherwise committed to.
- **Implementation guidance:** Define **screening** through its owner, scope, trigger, workflow, exceptions, and evidence.
...
For A.6.1, begin by identifying the specific risk, legal requirement, contractual commitment, or operational need that makes the control necessary. The control owner should then define what is in scope, which roles perform the activity, which systems or data sources are authoritative, how exceptions are handled, and what evidence proves that the control operated.

## Practical example

Before hiring a privileged system administrator, human resources verifies identity and relevant employment history under an approved screening level. An unresolved discrepancy is assessed by authorized roles and the decision is recorded confidentially.
