...
| CI | Configuration item, or continuous integration | Meaning depends on ITSM or software-delivery context |
| CI/CD | Continuous integration and continuous delivery | Automated software integration and delivery pipeline |
| CIA | Confidentiality, integrity, and availability | Core information-security objectives |
| CIO | Chief information officer | Executive role |
| CISA | Cybersecurity and Infrastructure Security Agency | United States government agency |
| CIS | Center for Internet Security | Publisher of security controls and benchmarks |
...
| IoT | Internet of Things | Connected devices and supporting services |
| IP | Intellectual property, or Internet Protocol | Meaning depends on legal/data or networking context |
| ISACA | Information Systems Audit and Control Association | Historical full name; the organization now uses ISACA as its name |
| ISB | Information Security Officer (*Informationssicherheitsbeauftragter*) | Role name used in German BSI-oriented guidance |
| ISMS | Information security management system | Management system defined by ISO/IEC 27001 |
| ISO | International Organization for Standardization | International standards organization |
| ISO/IEC | International Organization for Standardization and International Electrotechnical Commission | Jointly published international standards |
| IT | Information technology | Computing and supporting technology |
...
| PAM | Privileged access management | Management and control of privileged access |
| PCI | Payment Card Industry | Payment-card security context |
| PDCA | Plan-Do-Check-Act | Continual-improvement cycle |
| PET | Privacy-enhancing technology | Technology that reduces privacy exposure |
| PII | Personally identifiable information | Information linkable to an identifiable person |
| PDF | Portable Document Format | Document file format |
| PKI | Public key infrastructure | Certificates, keys, roles, and services supporting public-key cryptography |
| PQC | Post-quantum cryptography | Cryptography intended to resist quantum-computing attacks |
...
| SD-WAN | Software-defined wide area network | Centrally managed wide-area networking approach |
...
## Record prefixes and shorthand codes

The following strings are identifiers or compact table notation rather than general terminology. Their meanings are documented so a reader can interpret every code used in the project.
...
| `DATA-` | Data record |
| `DEC-` | Decision |
| `DSR-` | Data-security risk |
| `EFF-` | Effectiveness review |
| `EV-`, `EVD-` | Evidence |
| `EX-` | Exception |
| `F-` | Finding |
| `FLOW-` | Data flow |
...
| `OBJ-` | Objective |
| `OBL-` | Obligation |
| `P1`–`P4` | Priority levels 1 through 4; the organization must define which end of the scale is most urgent |
| `PROC-` | Process |
...
| `VULN-` | Vulnerability |
| `A/R` | A role that is both accountable and responsible |
| `I/C` | A role that is both informed and consulted |

## Usage guidance

The same letters can have different meanings. For example, **CI** can mean *configuration item* in service management and *continuous integration* in software delivery, while **IP** can mean *intellectual property* or *Internet Protocol*. Write the intended full form at first use whenever context does not make the meaning unambiguous.

Organization and framework names such as COBIT, ISACA, and ITIL are retained in their current common form. Their historical expansions are included to help readers, but the expansion should not be used to imply that it remains the organization's or framework owner's formal name.
